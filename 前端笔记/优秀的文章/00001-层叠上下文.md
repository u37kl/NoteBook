...
这篇文章发布于 2016年01月9日，星期六，22:15，归类于 [CSS相关](https://www.zhangxinxu.com/wordpress/category/css/)。 阅读 155787 次, 今日 53 次 [74 条评论](https://www.zhangxinxu.top/wordpress/2016/01/understand-css-stacking-context-order-z-index/#comments)
...
![网页中z轴示意](./media/00001-层叠上下文/z-aris.png)
...
![你懂的](./media/00001-层叠上下文/point.gif)
...
![层叠顺序](./media/00001-层叠上下文/2016-01-07_223349.png)
...
![疑问](./media/00001-层叠上下文/ask.gif)

为什么呢？我明明感觉浮动元素和块状元素要更屌一点啊。

嘿嘿嘿，我就不卖关子了，直接看下图的标注说明：
![层叠顺序元素的标注说明](./media/00001-层叠上下文/2016-01-07_235108.png)
...
![浮动和文字重叠](./media/00001-层叠上下文/2016-01-07_235830.jpg)
...
<img src="./media/00001-层叠上下文/image-20210119211040614.png" width=300>
...
<img src="./media/00001-层叠上下文/image-20210119211352962.png" width=300>

大家会发现，尼玛反过来了，竖着的妹子(mm2)这回趴在了横着的妹子(mm1)身上。

为什么小小的改变会有想法的结果呢？
![思考](./media/00001-层叠上下文/think-2.gif)
...
<img src="./media/00001-层叠上下文/image-20210119211524052.png" width=300>



会发现，妹子跑到蓝色背景的下面了。为什么呢？层叠顺序图可以找到答案，如下：
![负值z-index的层叠顺序](./media/00001-层叠上下文/2016-01-08_235511.png)
...
<img src="./media/00001-层叠上下文/image-20210119211620047.png" width=300>



会发现，妹子在蓝色背景上面显示了，为什么呢？层叠顺序图可以找到答案，如下：
![img](./media/00001-层叠上下文/2016-01-08_235217.png)
...
<img src="./media/00001-层叠上下文/image-20210119211759717.png" width=300>
...
<img src="./media/00001-层叠上下文/image-20210119211844396.png" width=300>
...
<img src="./media/00001-层叠上下文/image-20210119212714181.png" width=500>
...
<img src="./media/00001-层叠上下文/image-20210119212745633.png" width=300>
...
<img src="./media/00001-层叠上下文/image-20210119212928236.png" width=300>
...
<img src="./media/00001-层叠上下文/image-20210119213100042.png" width=300>
...
<img src="./media/00001-层叠上下文/image-20210119213214287.png" width=300>
...
![更完整的7阶层叠顺序图](./media/00001-层叠上下文/2016-01-09_211116.png)
...
<img src="./media/00001-层叠上下文/image-20210119213436931.png" width=300>

会发现，两者样式一模一样，仅仅是在DOM流中的位置不一样，导致他们的层叠表现不一样，后面的妹子趴在了前面妹子的身上。这也说明了，层叠上下文元素的层叠顺序就是`z-index:auto`级别。

**z-index值与层叠顺序**
如果元素支持z-index值，则层叠顺序就要好理解些了，比较数值大小嘛，小盆友都会，本质上是应用的“谁大谁上”的准则。在以前，我们只需要关心定位元素的z-index就好，但是，在CSS3时代，flex子项也支持`z-index`，使得我们面对的情况比以前要复杂。然而，好的是，规则都是一样的，对于`z-index`的使用和表现也是如此，套用上面的7阶层叠顺序表就可以了。
...
<img src="./media/00001-层叠上下文/image-20210119214206155.png" width=300>
...
![文字在妹子上](./media/00001-层叠上下文/2018-01-09_004641.png)

但是，一旦图片开始走fadeIn淡出的CSS3动画，文字跑到图片后面去了![img](./media/00001-层叠上下文/36.gif)：

![文字跑到图片后面](./media/00001-层叠上下文/2018-01-09_004654.png)
...
> stacking level(层叠等级)和stacking order(层叠顺序)很容易混淆，stacking order定义的是覆盖规则，而stacking level是覆盖等级，每个元素都有stacking level，根据规则算出自己的覆盖等级，覆盖等级小的会被覆盖。
>
> 误区：z-index不是stack level，上面说了stack level所有元素都有，而z-index只有定位的元素才有。
>
> 
>
> DOM对象的层叠等级比较：
>
> 1. 处于同一个层叠上下文中的DOM元素才能比较否则无法比较，那么找包裹它们的外层DOM元素，直到同于同一个层叠上下文的两个DOM元素，然后进行比较，如果其中一个大于另一个，则它的所有子节点DOM都大于另一个(`比较必须使用同一个参照物，这种比较的逻辑，儿子比较找不到参照物就比爹，知道找到同一个参照物`)。
>
> 2. DOM对象有两种一个是普通DOM对象，一个是变成层叠上下文的DOM对象，默认变成层叠上下文的DOM对象的层叠顺序在(第5层)，但是有些层叠上下文DOM对象自己通过z-index修改层叠顺序(`例如定位+z-index，z-index>0在第6层，z-index<0在第1层`)
> 3. 层叠等级相同时，看编码顺序，后写的DOM对象放在上面。
> 4. 在层叠规则中还有一个重要条件，父子层叠关系，例如当父元素开启定位时，子元素还是block，此时应该子元素比父元素层叠等级低，被父元素覆盖，但是真实情况并没有，因此只要子元素没有`开启定位并将z-index设置负值`，子元素层叠等级就比父元素的大。`在比较子元素层叠等级时，先比较父元素层叠等级，如果层叠等级相同(父元素层叠关系根据代码编写顺序来决定)，再比较子元素层叠等级`

<img src="media/00001-层叠上下文/image-20210409163929283.png" width="400">
...
>
> 1. html标签就是一个层叠上下文
> 2. z-index值为数字的定位DOM是层叠上下文。
> 3. css3中新增的层叠上下文
>    1. opacity不为1的DOM对象是层叠上下文。
>    2. 父DOM为flex布局，子DOM对象z-index值为数字时，`子DOM对象是层叠上下文`
>    3. tranform值不为none的DOM元素是层叠上下文。
>    4. 元素`mix-blend-mode`值不是`normal`是层叠上下文
>    5. 元素的`filter`值不是`none`，是层叠上下文
>    6. 元素的`isolation`值是`isolate`，是层叠上下文
>    7. `will-change`指定的属性值为上面任意一个，是层叠上下文
>    8. 元素的`-webkit-overflow-scrolling`设为`touch`，是层叠上下文



### BFC元素与外部元素的层叠比较

> 代码运行后，center元素的内部的子元素为block，right元素也是block，按照层叠顺序应该right元素覆盖center元素的子元素，但结果是反过来。
>
> 分析：对于BFC元素内部元素与外部元素的比较，如果处于同一个层叠上下文中，
>
> 1. 首先外部元素与BFC元素进行比较，如果外部元素层叠等级>BFC元素，则在与内部元素进行比较，
> 2. 如果还是大于，则外部元素覆盖内部子元素，反之内部元素覆盖外部元素。
> 3. 如果内部元素比开启BFC的父元素的层叠等级小，则比较外部元素的层叠等级于内部元素，一般这种情况子元素`开启定位并将z-index设置为负值`，这样就会比BFC父元素的层叠等级小。
...
<img src='media/00001-层叠上下文/image-20210412115420953.png' width=400>
...
> 比较两个元素覆盖等级：
>
> 1. 首先看两个元素是否处在同一个父元素中，如果不在的话，找到它们各自处于同一个祖先元素的，下一辈元素，比较这两个元素的覆盖等级，覆盖等级小的将覆盖被覆盖。
> 2. 上面步骤就是为了实现在同一个层叠上下文中比较，各个元素的层叠等级都是基于最近的祖先元素的层叠上下文+层叠规则序号获得层叠等级(实际上不是，好记)
> 3. 子元素层叠等级，序号越高等级越高
>    1. 父元素层叠上下文的background/border
>    2. index<0
>    3. block
>    4. float
>    5. inline、inline-block
>    6. 定位并且(z-index: auto或z-index: 0) 或者 当前元素是一个层叠上下文
>    7. z-index > 0
> 4. 当两个元素层叠等级相同时，按照代码编写顺序，手写的覆盖先写的。

```html
<!--
	div的层叠等级 = 最近的祖先元素的层叠上下文(html = 0) + 3(上面的序号，div是block，因此为3) = 3
	p的层叠等级 =  最近的祖先元素的层叠上下文(html = 0, div没有层叠上下文) + 2
因此 div覆盖p，
-->
<body>
	<div style="background-color:red">
		<p style="position:relative; z-index:-1">aaaa</p>
	</div>
  
  <!--
	div的层叠等级 = 最近的祖先元素的层叠上下文(html = 0) + 3(上面的序号，div是block，因此为3) = 3
	p的层叠等级 =  最近的祖先元素的层叠上下文(html = 0, div没有层叠上下文) + 6 = 6
	因此 p覆盖div，
	-->
  	<div style="background-color:red">
		<p style="position:relative;">aaaa</p>
	</div>
</body>

```

![image-20210119222103536](./media/00001-层叠上下文/image-20210119222103536.png)

## 慎用z-index

> z-index很容易导致覆盖紊乱，原因在于定位元素，在设置z-index属性不为auto时，该元素会生成一个层叠上下文，像下面两段代码，父元素开启层叠，直接将子元素的z-index废掉。
>
> 如果你用z-index，尽量不要出现处于不同父元素的子元素进行覆盖操作，下面的例子就是，两个子元素互不相干，还调整子元素的z-index进行覆盖，典型的奇葩逻辑
